...
export default function Input({ label , type, value, onChange, error,classname,name,customerror } : InputProps){
    ...
    return (
      <div>
        ...
        <input type={type} value={value} onChange={onChange} className={`${className} ${error ? 'invalid' : ''}`} name={name}/>
        ...
        { customerror && <small className="text-red-400 dark:brightness-125 text-[0.785rem] font-medium block mt-1 pl-3 leading-snug" >{customerror}</small> }
